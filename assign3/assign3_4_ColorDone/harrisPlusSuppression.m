...
function [cim] = harrisPlusSuppression(im, sigma, size, thresh, radius, disp)
    %r, c
    narginchk(3,6);
...
%   cim = (Ix2.*Iy2 - Ixy.^2) - k*(Ix2 + Iy2).^2; 
    ...
    cim = imresize(cim, size); 
	sze = 2 * radius+1;                       % Size of mask.
    mx = ordfilt2( cim, sze^2, ones(sze) );   % Find maxima.
	cim = cim .* (cim == mx) .* (cim > thresh);
