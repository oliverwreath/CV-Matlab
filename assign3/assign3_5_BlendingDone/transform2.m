function [ imgDouble, tform, tform2, xdata, ydata ] = transform2( fileName )
...
for i = 1 : numberOfImages
    ...
    for j = 1 : numberOfSupsampling
        ...
        [ cim{j} ] = harrisPlusSuppression( tempSupImage, 1, size(imgDouble{i}), 0.03, 8, 0 );
    end
%     for j = 1 : numberOfSupsampling
%         cim{j}
%     end
...
B = B( B ~= 0 );
B = sort( B(:), 'ascend' );
threshold = B( min( thresNum + 1, size(B, 1)) );
% [r,c,v] = find(X>2)
...
%                             n2 = dist2( featurePoint{2}, hypoXY );
    ...
    if affine == 1 % affine
        for i = 1 : 1 % numberOfImages
            ...
            counter = 1 ;
            RANSACrounds = min( RANSACrounds, numOfFeatures .^ 3 / factorial(3) / 2 )
    %             for rounds = 1 : RANSACrounds
            while 1 == 1
                ...
                if counter > RANSACrounds
                    break;
                elseif flagMatrix( j1, j2, j3 ) == true
                    continue;
                else
    ...
    %                 n2 = pdist2( featurePoint{2}, hypoXY, 'euclidean' );
        ...
        for i = 1 : 1 % numberOfImages
            ...
            counter = 1 ;
            RANSACrounds = min( RANSACrounds, numOfFeatures .^ 4 / factorial(4) / 2 )
            while 1 == 1
%             for rounds = 1 : RANSACrounds
                ...
                if counter > RANSACrounds
                    break;
                elseif flagMatrix( j1, j2, j3, j4 ) == true
                    continue;
                else
                    flagMatrix( j1, j2, j3, j4 ) = true;
                    counter = counter + 1;
                end
                % get X from AX=0
                A = [ ...
...
%     tform = maketform('projective', inlier_input_points( 1: 4, : ), inlier_base_points( 1: 4, : ) );
    ...
    tform2 = maketform( 'projective', [1     0      0 ;...
         0    1    0 ;...
         0    0    1] );
end

[ ~, xdata, ydata ] = imtransform( imgDouble{1}, tform );
