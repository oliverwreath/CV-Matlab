function [ merge_output, xdata, ydata ] = transform5( fileName )
...
tic1 = tic;
    ...
    [ imgDouble_5, tform_5, ~, xdata_5, ydata_5 ] = transform2( [  fileName(5, :); fileName(3, :) ] );
    
    min3 = @( x, y, z ) min( min( x, y ), z );
    max3 = @( x, y, z ) max( max( x, y ), z );
    min4 = @( x, y, m, n ) min( min( x, y ), min( m, n ) );
    max4 = @( x, y, m, n ) max( max( x, y ), min( m, n ) );
    xdata(1) = min4( xdata_1(1), xdata_2(1), xdata_4(1), xdata_5(1) );
    ydata(1) = min4( ydata_1(1), ydata_2(1), ydata_4(1), ydata_5(1) );
    xdata(2) = max4( xdata_1(2), xdata_2(2), xdata_4(2), xdata_5(2) );
    ydata(2) = max4( ydata_1(2), ydata_2(2), ydata_4(2), ydata_5(2) );
    ...
    [ output5, ~, ~ ] = imtransform( imgDouble_5{1}, tform_5, 'XData', xdata, 'YData', ydata );
    % figure, imshow( output2, 'XData', xdata , 'YData', ydata ), axis on, impixelinfo;

    merge_output = output1 .* 0.3 + output2 .* 0.3 + output3 .* 0.3 + output4 .* 0.3 + output5 .* 0.3;
