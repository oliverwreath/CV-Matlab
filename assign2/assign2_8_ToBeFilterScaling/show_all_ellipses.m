...
%     eigD( :, 1) = ( eigD( :, 1) - min(min( eigD( :, 1) )) ) ./ max(max( eigD( :, 1) ));
%     eigD( :, 2) = ( eigD( :, 2) - min(min( eigD( :, 2) )) ) ./ max(max( eigD( :, 2) ));
    thresHold = 1.2;
    level1 = (graythresh( eigD( :, 1) )+0.001) * thresHold;
    level2 = (graythresh( eigD( :, 2) )+0.001) * thresHold;
